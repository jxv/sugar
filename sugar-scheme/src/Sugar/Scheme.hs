...
tokenToLispVal (_, P.Text txt G.HasQuote _) = pure $ String txt
tokenToLispVal (loc, P.Text txt G.NoQuote _) =  case parse valParser "valParser" txt of
  Left _ -> Left (loc,txt)
  Right v -> Right v
...
sugarToLispVal (G.Map m _) = (HashTable . Map.fromList) <$> mapM (\(x,y) -> (,) <$> sugarToLispVal x <*> sugarToLispVal y) m
